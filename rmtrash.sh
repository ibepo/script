#!/bin/bash
# Author            : ibepo
# Date              : 2022-03-16 11:17:10
# Last Modified Date: 2022-03-16 11:21:43
# Last Modified By  : ibepo
...
if [[ $return_value -ne 0 ]] ;then
	...
	echo "alias rm=~/dotfiles/script/rmtrash.sh" >>$alias_file && source $alias_file
###如果存在rm alias，且不是指向rmtrash的，则注释掉，区分linux 和mac
elif [[ "$alias_rm" != "alias rm=~/dotfiles/script/rmtrash.sh" ]];then
	echo already has alias rm,and must commit out
	if [[ $os_type == Darwin ]];then
		sed -i .bak 's/^alias\ rm=/#alias\ rm=/g' $alias_file && \
		echo "alias rm=~/dotfiles/script/rmtrash.sh" >>$alias_file && \
		source $alias_file
